...
const fetchAllEvents = async (req, res, next) => {
    try {
        const eventDetails = await eventSchema.find().populate("venueId");
        if (!eventDetails) {
            throw {
                ...errors[404],
...
const createEvent = async (req, res, next) => {
    try {
        ...
        let { category, name, description, image, eventDate, tags, vipPrice, gaPrice, city, slotDetails, venueId } = req.body;
        ...
        const event = new eventSchema({ category, name, description, image,eventDate, tags, vipPrice, gaPrice, city, slotDetails, venueId });
